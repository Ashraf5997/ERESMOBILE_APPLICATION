  ...
  const ADDTASK=()=>{
    if(inputText == 0)
      ...
      let TodoData =
      {
           ...
           axios.post("http://192.168.0.100:9000/api/v1/e-res/createTask",TodoData).then(
                (response)=>{
...
const getAllTaskById= ()=>{
  setDNF(false);
  axios.get(`http://192.168.0.100:9000/api/v1/e-res/taskById/${userid}`).then(
     (response)=>{
      // jan
...
/* // FETCHING ALL TASK   IN  EACH  MONTHS
 ...
 // GET ALL DAYS BY A MONTH
  const getDaysByMonth = (month)=>{
    ...
    axios.post("http://192.168.0.100:9000/api/v1/e-res/getDuplicateDayById",dataobj).then(
    (response)=>{
